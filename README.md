...
本项目现在只有一个预约事实来源：`tasks.json`。

- `config.ini` / `configs/*.ini`：只写账号、通知、OCR、超级鹰兜底等通用配置。
- `tasks.json`：写任务名、指向哪个 config、浏览器、场馆、场地号、`start_time`、`end_time`、重试参数、是否启用。
- 调度器计算开放时间时，会读取 task 自己的 `start_time`；WebUI 保存 config 时会移除旧的 `[type]` 和 `[time]`。

## 相对原仓库的主要改动

- `config0.ini` 示例已改为 `config.example.ini`；实际使用时复制为 `config.ini`。`config.ini` 默认被 `.gitignore` 忽略。
- `duration` 已删除，预约长度只由 task 里的 `start_time` 和 `end_time` 决定。
...
- 新增 `booking_scheduler.py`：按“目标日期前 3 天 12:00 开放预约”的规则提前启动登录预热，并让 `main.py` 在登录后等到 12:00:00 再进入预约页。
- 新增 Flask WebUI：可编辑本地配置、创建多个预约任务、启动/停止调度器。
- 微信通知已从 Server酱迁移到 [PushPlus](https://www.pushplus.plus/push1.html)，配置不变，只需在 PushPlus 网站登录获取 token 填入 `config.ini` 的 `[wechat] SCKEY` 即可。

## 安装

建议新建独立的 `autobook` conda 环境，避免和已有推理、训练环境互相污染：

```bash
conda create -n autobook python=3.11 -y
...
启动 WebUI 后，所有配置都可在浏览器界面完成，无需手动编辑配置文件。
...
- 读取、编辑、保存 `config.ini`（账号、通知、OCR 配置）。
- 一键从配置生成预约任务（场馆、时段）。
- 管理 `tasks.json` 中的多个预约任务。
- 启动/停止调度器，查看运行状态和日志。
...
- `[glm_ocr] timeout`：默认 `20` 秒。本地 GLM-OCR 在高峰期可能需要十几秒返回，低于这个值容易把可解验证码误记成超时失败。

场馆和时间写在任务里：

- `venue`：推荐写完整父子场馆名，例如 `五四体育中心-羽毛球馆` 或 `邱德拔体育馆-羽毛球场`。
- `venue_num`：指定场地编号填正整数；随机场地填 `-1`。
- `start_time/end_time`：支持 `YYYYMMDD-HHMM` 或 `星期-HHMM`，星期格式中 `1` 到 `7` 表示周一到周日。调度器里如果星期等于今天，会按下周同一天处理。
...
如果 OCR 服务已经在 `tmux` 会话里跑着，直接连回去看日志：
...
python main.py --config config.ini --browser firefox --once \
  --venue 五四体育中心-羽毛球馆 --venue-num -1 \
  --start-time 20260422-0650 --end-time 20260422-0750
```
...
预约开放规则按“目标日期前 3 天 12:00”计算。例如 `2026-04-22` 最早在 `2026-04-19 12:00` 可预约。

`lead_seconds` 表示登录预热秒数，不表示提前点击预约。默认 `60` 的行为是：

- `11:59:00` 启动 `main.py`，打开浏览器并完成门户登录。
- 登录完成后 `main.py` 等到 `12:00:00`。
- `12:00:00` 到达后才进入场馆页面、查找空闲场地并提交订单。

因此日志中的失败时间可能是 `12:00:25` 之类的后续完成时间，不代表第一次进入预约页的时间。
现在 `config.log` 还会额外记录 `到达预约开放时间`、`开始进入预约 ... 界面`、`开始查找空闲场地` 三个精确时间点，便于确认真正的起跑时刻。

当 `venue_num = -1` 时，程序不再按 `1, 2, 3...` 顺扫场地，而是基于账号、场馆、日期和时段预计算一个确定性扫描顺序，优先避开低编号，减少和其他脚本默认顺扫时撞在同一块场地上的概率。
...
同一时段同时抢五四和邱德拔：在 `tasks.json` 中写两个任务，可以复用同一个 `config.ini`。调度器会并发运行所有到点任务。
