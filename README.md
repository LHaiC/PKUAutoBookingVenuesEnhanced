...
- 用“日期或星期 / 开始时间 / 至少几小时”生成 task 的 `start_time/end_time`；原始时间字段折叠在“高级时间配置”里，供多个候选时段手动编辑。
- 创建多个未来预约任务。任务选择对应 config，并在任务里保存场馆和预约时间。
- 任务表中“开抢时间”是实际进入预约页的时间；“预热启动”是提前启动登录的时间，由“登录预热秒数”控制。
